...
# Noms de lieux à masquer partout (peu importe le contexte)
ALWAYS_MASK_SINGLE = {
    "rangueil",
    "pasteur",
    "ormeau",
...
# Syntagmes multi-mots à masquer partout (liste de listes de tokens en minuscules)
ALWAYS_MASK_MULTI = [
    ["bagneres", "de", "bigorre"],
    ["bagnères", "de", "bigorre"],
    ["vic", "en", "bigorre"],
...
def detect_pii_boxes_from_ocr(ocr: dict) -> List[Tuple[int, int, int, int]]:
    ...
    # On ordonne les lignes pour pouvoir gérer les cas sur 2 lignes
    line_items = sorted(
        line_dict.items(),
    ...
    for key, line in line_items:
        ...
        # b) GROUPES NUMÉRIQUES CONSÉCUTIFS (numéros de téléphone, etc.)
        i = 0
        while i < len(words):
        ...
        # c) NOMS DE LIEUX À MASQUER PARTOUT (formes simples + multi-mots)

        #  c1. formes simples (1 mot)
        for w in words:
            low = normalize_word(w["text"]).lower()
            if low in ALWAYS_MASK_SINGLE:
                boxes.append(_bbox_word(w))

        #  c2. formes multi-mots ("Bagneres de Bigorre", "Vic en Bigorre", etc.)
        i = 0
        while i < len(words):
            low_i = normalize_word(words[i]["text"]).lower()
            matched = False
        ...
        # d) NOMS DE PATIENTS (sur la même ligne) après Monsieur/Madame/etc.
        ...
        # e) NOMS DE MÉDECINS (sur la même ligne) après Docteur/Dr
        for i, w in enumerate(words):
            low = normalize_word(w["text"]).lower()
        ...
        # f) ADRESSES : "<num> rue/route/boulevard/..." → du numéro à la fin de la ligne
        for i, w in enumerate(words):
        ...
        # g) ÉTABLISSEMENTS : Hôpital/Hopital/Hôpitaux, Centre hospitalier, CH, CHU, Clinique
        for i, w in enumerate(words):
    ...
    for idx, (key, line) in enumerate(line_items):
        ...
        if low_last in TITLE_PATIENT or low_last in TITLE_DOCTOR:
            if idx + 1 < len(line_items):
                ...
                # on peut vérifier qu'on reste sur la même page
                if next_key[0] == key[0]:
